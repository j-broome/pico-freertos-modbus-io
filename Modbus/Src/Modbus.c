...
void ModbusInit(modbusHandler_t * modH)
{
	if (numberHandlers < MAX_M_HANDLERS)
    {
	    ...
	    {		   		
		    xTaskCreate ((TaskFunction_t)StartTaskModbusSlave, "ModbusSlave", (uint16_t)(STACK_SIZE), modH, osPriorityLow, &modH->myTaskModbusAHandle);
	    ...
	    {
	        xTaskCreate ((TaskFunction_t)StartTaskModbusMaster, "ModbusMaster", (uint16_t)(STACK_SIZE), modH, osPriorityLow,  &modH->myTaskModbusAHandle);
			if( modH->myTaskModbusAHandle == NULL) 
		    {
		       while(1); //error creating modbus task
		    }
...
static void sendTxBuffer(modbusHandler_t *modH)
{
	...
	//RingClear(&modH->xBufferRX); // clear rx buffer
